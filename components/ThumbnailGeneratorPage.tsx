        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-lime-800 rounded-xl shadow-2xl w-full max-w-lg border border-lime-700">
                <div className="p-6">
                    ...
                    <p className="text-lime-300/70 mt-2">Dán API key, mỗi key một dòng. Ứng dụng sẽ tự động xoay vòng khi hết hạn mức.</p>
                    <textarea value={keysInput} onChange={(e) => setKeysInput(e.target.value)} placeholder="AIzaSy..." rows={6} className="w-full mt-4 p-2 bg-lime-900 border border-lime-600 rounded-md text-lime-200 font-mono placeholder-lime-600" />
                </div>
...
const ThumbnailGeneratorPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    return (
        <div className="min-h-screen bg-lime-950 flex flex-col p-4 sm:p-8 lg:p-12 animate-fadeInUp">
            ...
            <main className="flex-grow flex flex-col gap-4 overflow-hidden">
                <div className="flex-shrink-0">
                    ...
                    <p className="text-lime-300/70 mt-1">
                ...
                <div className="flex-grow overflow-hidden">
                    <div className="bg-lime-900 text-lime-100 p-3 sm:p-6 rounded-lg border border-lime-700 shadow-2xl h-full flex flex-col gap-6">
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 flex-grow overflow-hidden">
                            <div className="lg:col-span-1 flex flex-col gap-6 overflow-y-auto pr-2">
                                <div className="bg-lime-950/50 p-4 rounded-lg border border-lime-700 space-y-4">
                                    <h3 className="text-lg font-bold text-lime-100">1. Tùy chỉnh</h3>
                                    
                                     <div className="space-y-2 bg-lime-800/50 p-3 rounded-md border border-lime-600">
                                        ...
                                        <div className="space-y-1">
                                            {apiKeys.length > 0 ? apiKeys.slice(0, 3).map(key => {
                                                ...
                                                return (
                                                    <div key={key} className="flex items-center justify-between p-2 rounded bg-lime-900 text-xs border border-lime-700">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex space-x-2">
                                            {['youtube', 'tiktok', 'facebook'].map(p => (
                                                <button key={p} onClick={() => setPlatform(p)} className={`px-4 py-2 rounded-md font-bold text-sm transition-all duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-lime-800 ${platform === p ? 'bg-blue-600 text-white ring-2 ring-blue-400' : 'bg-lime-600 text-white hover:bg-lime-700 focus:ring-lime-500'}`}>{p.charAt(0).toUpperCase() + p.slice(1)}</button>
                                            ))}
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div 
                                            ...
                                            onDrop={handleDrop}
                                            className="w-full bg-lime-800 rounded-md border-2 border-dashed border-lime-600 transition-colors p-2"
                                        >
                                            <input 
                                                ref={fileInputRef} 
                                                hidden 
                                            ...
                                            {imagePreviews.length > 0 ? (
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                                    ...
                                                    <button onClick={handleUploadClick} className="flex flex-col items-center justify-center bg-lime-900/50 rounded-md aspect-square text-lime-500 hover:text-lime-300 hover:border-blue-500 border-2 border-dashed border-lime-700 transition-colors">
                                                ...
                                                <div 
                                                    ...
                                                    className="min-h-[120px] flex flex-col items-center justify-center cursor-pointer hover:bg-lime-700/50 rounded-md"
                                                >
                                                    <div className="flex flex-col items-center text-lime-500 pointer-events-none">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex space-x-2">
                                            {[1, 2, 3, 4].map(n => (
                                                <button key={n} onClick={() => setImageCount(n)} className={`px-4 py-2 rounded-md font-bold text-sm transition-all duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-lime-800 flex-1 ${imageCount === n ? 'bg-blue-600 text-white ring-2 ring-blue-400' : 'bg-lime-600 text-white hover:bg-lime-700 focus:ring-lime-500'}`}>{n}</button>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                                <div className="bg-lime-900/50 p-4 rounded-lg border border-lime-700 space-y-4">
                                    <h3 className="text-lg font-bold text-lime-100">2. Nhập nội dung</h3>
                                    <div>
                                        <label className="block text-sm font-semibold mb-2">Chế độ tạo ảnh</label>
                                        <div className="flex space-x-2">
                                            {['accurate', 'creative'].map(m => (
                                                <button key={m} onClick={() => setGenerationMode(m)} className={`px-4 py-2 rounded-md font-bold text-sm transition-all duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-lime-800 ${generationMode === m ? 'bg-blue-600 text-white ring-2 ring-blue-400' : 'bg-lime-600 text-white hover:bg-lime-700 focus:ring-lime-500'}`}>{m === 'accurate' ? 'Chính xác' : 'Sáng tạo'}</button>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <label className="text-sm font-semibold">Hiển thị chữ trên Thumbnail</label>
                                        <button onClick={() => setShowText(!showText)} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${showText ? 'bg-blue-600' : 'bg-lime-600'}`}>
                                    ...
                                    <div>
                                        ...
                                        <textarea id="text-prompts" value={textPrompts} onChange={e => setTextPrompts(e.target.value)} className="w-full h-24 bg-lime-800 text-lime-100 border border-lime-600 rounded-md p-2 font-mono text-sm placeholder-lime-500" placeholder="Ví dụ: BÍ MẬT GIẢM CÂN ĐỘT PHÁ CÔNG HOA HỒNG 2024 REVIEW SẢN PHẨM MỚI"></textarea>
                                    </div>
                                    <div>
                                        <label htmlFor="creative-suggestion" className="block text-sm font-semibold mb-1">Ý tưởng sáng tạo (Tùy chọn)</label>
                                        <textarea id="creative-suggestion" value={creativeSuggestion} onChange={e => setCreativeSuggestion(e.target.value)} className="w-full h-20 bg-lime-800 text-lime-100 border border-lime-600 rounded-md p-2 font-mono text-sm placeholder-lime-500" placeholder="tông màu vàng, bên trái..."></textarea>
                                    </div>
                                    <button onClick={handleGenerateThumbnails} disabled={isGenerationDisabled} className="flex items-center justify-center gap-2 px-4 py-2 rounded-md font-bold transition-all duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-lime-800 disabled:opacity-50 disabled:cursor-not-allowed bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-500 w-full mt-2 text-lg">
                                        {isGenerating ? <><Loader2 className="w-6 h-6 animate-spin"/> Đang tạo...</> : 'Tạo hình thu nhỏ'}
                                    </button>
                                </div>
                            </div>
                            <div className="lg:col-span-3 flex flex-col">
                                <h3 className="text-lg font-bold text-lime-100 mb-2">Kết quả</h3>
                                <div className="bg-lime-900/50 border border-lime-700 rounded-lg p-4 flex-grow overflow-y-auto">
                                     {isGenerating ? (
                                        <div className="flex flex-col items-center justify-center h-full text-lime-400">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {generatedImages.map((img, idx) => (
                                                <div key={idx} className="relative group">
                                                    <img src={`data:image/jpeg;base64,${img}`} alt={`Generated ${idx}`} className="w-full h-auto rounded-lg shadow-lg border border-lime-600"/>
                                                    <a 
                                                        href={`data:image/jpeg;base64,${img}`} 
                                                        download={`thumbnail-${idx + 1}.jpg`}
                                                        className="absolute bottom-2 right-2 p-2 bg-lime-700/80 hover:bg-lime-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                                                        title="Tải xuống"
                                                    >
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full text-lime-500 opacity-50">
