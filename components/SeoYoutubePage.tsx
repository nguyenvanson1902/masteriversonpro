...
const SeoYoutubePage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
            ...
            <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                <div className="bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg border border-slate-700">
                    <div className="p-6">
                        ...
                        <textarea value={keysInput} onChange={(e) => setKeysInput(e.target.value)} placeholder="Dán API key, mỗi key một dòng..." rows={6} className="w-full mt-4 p-2 bg-slate-900 border border-slate-600 rounded-md text-gray-200 font-mono" />
                    </div>
                    <div className="bg-slate-900/50 px-6 py-3 rounded-b-xl flex justify-end gap-3">
                        ...
                        <button onClick={() => onSave(keysInput.split(/[\n,]+/).map(k => k.trim()).filter(Boolean))} className="px-5 py-2 bg-blue-600 text-white font-bold rounded-lg">Lưu</button>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-gray-100 flex flex-col p-4 sm:p-8 animate-fadeInUp">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
                        <div className="lg:col-span-1 flex flex-col gap-8 overflow-y-auto pr-2">
                            <div className="bg-gray-800 p-6 rounded-xl border border-gray-700">
                                ...
                                <textarea 
                                    className="w-full h-32 bg-gray-900 border border-gray-600 rounded-lg p-3 text-base text-gray-200 focus:ring-2 focus:ring-blue-500 transition-colors" 
                            ...
                            {suggestedTitles.length > 0 && (
                                <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 animate-fadeInUp">
                                    <h2 className="text-xl font-bold mb-4 text-cyan-400">Bước 2: Chọn tiêu đề bạn thích</h2>
                                    <div className="space-y-3">
                                        {suggestedTitles.map((title, index) => (
                                            <div key={index} className={`flex items-center justify-between p-3 rounded-lg transition-all duration-200 border-2 ${selectedTitle === title ? 'bg-blue-900/50 border-blue-500' : 'bg-gray-700 border-gray-600'}`}>
                                                <span className="flex-grow mr-4 text-sm">{title}</span>
                                                <button onClick={() => handleSelectTitle(title)} className={`flex-shrink-0 px-4 py-1 text-sm font-bold rounded-md transition-colors ${selectedTitle === title ? 'bg-blue-600 text-white cursor-default' : 'bg-slate-600 hover:bg-blue-500 text-white'}`}>
                                    ...
                                    {seoContent ? (
                                        <div className="space-y-6 animate-fadeInUp">
                                            <div className="relative bg-gray-900 p-4 rounded-lg border border-gray-700">
                                                ...
                                                <p className="text-gray-300 whitespace-pre-wrap">{seoContent.description}</p>
                                                <button onClick={() => handleCopy(seoContent.description, 'desc')} className="absolute top-2 right-2 px-3 py-1 text-xs rounded-md transition-colors duration-200 bg-gray-600 hover:bg-gray-500 text-gray-200 flex items-center gap-1.5">{copySuccess['desc'] ? <CheckCircleIconLucide className="w-3 h-3 text-green-400" /> : <ClipboardIconLucide className="w-3 h-3" />} {copySuccess['desc'] ? 'Đã chép' : 'Sao chép'}</button>
                                            </div>
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                <div className="relative bg-gray-900 p-4 rounded-lg border border-gray-700 md:col-span-1">
                                                    <h3 className="font-semibold text-lg mb-2 text-purple-400">Hashtags</h3>
                                                    <div className="flex flex-wrap gap-2">
                                                        {seoContent.hashtags.map((tag, i) => <span key={i} className="bg-gray-700 text-gray-200 px-2 py-1 rounded text-sm">#{tag}</span>)}
                                                    </div>
                                                    <button onClick={() => handleCopy(seoContent.hashtags.map(t => `#${t}`).join(' '), 'tags')} className="absolute top-2 right-2 px-3 py-1 text-xs rounded-md transition-colors duration-200 bg-gray-600 hover:bg-gray-500 text-gray-200 flex items-center gap-1.5">{copySuccess['tags'] ? <CheckCircleIconLucide className="w-3 h-3 text-green-400" /> : <ClipboardIconLucide className="w-3 h-3" />} {copySuccess['tags'] ? 'Đã chép' : 'Sao chép'}</button>
                                                </div>
                                                <div className="relative bg-gray-900 p-4 rounded-lg border border-gray-700 md:col-span-2">
                                                    <h3 className="font-semibold text-lg mb-2 text-yellow-400">Từ khóa chính</h3>
                                                    <div className="flex flex-wrap gap-2">
                                                         {seoContent.primaryKeywords.map((kw, i) => <span key={i} className="bg-gray-700 text-gray-200 px-2 py-1 rounded text-sm">{kw}</span>)}
                                                    </div>
                                                     <button onClick={() => handleCopy(seoContent.primaryKeywords.join(', '), 'primary')} className="absolute top-2 right-2 px-3 py-1 text-xs rounded-md transition-colors duration-200 bg-gray-600 hover:bg-gray-500 text-gray-200 flex items-center gap-1.5">{copySuccess['primary'] ? <CheckCircleIconLucide className="w-3 h-3 text-green-400" /> : <ClipboardIconLucide className="w-3 h-3" />} {copySuccess['primary'] ? 'Đã chép' : 'Sao chép'}</button>
                                                </div>
                                            </div>
                                            <div className="relative bg-gray-900 p-4 rounded-lg border border-gray-700">
                                                <h3 className="font-semibold text-lg mb-2 text-orange-400">Từ khóa phụ</h3>
                                                <div className="flex flex-wrap gap-2">
                                                    {seoContent.secondaryKeywords.map((kw, i) => <span key={i} className="bg-gray-700 text-gray-200 px-2 py-1 rounded text-sm">{kw}</span>)}
                                                </div>
                                                <button onClick={() => handleCopy(seoContent.secondaryKeywords.join(', '), 'secondary')} className="absolute top-2 right-2 px-3 py-1 text-xs rounded-md transition-colors duration-200 bg-gray-600 hover:bg-gray-500 text-gray-200 flex items-center gap-1.5">{copySuccess['secondary'] ? <CheckCircleIconLucide className="w-3 h-3 text-green-400" /> : <ClipboardIconLucide className="w-3 h-3" />} {copySuccess['secondary'] ? 'Đã chép' : 'Sao chép'}</button>
                                            </div>
                                        </div>
                                    ) : (
                                        !isGeneratingContent &&
